...
class AtomTypeCounts(object):
    ...
    def parsePDB(self, rec_sele="protein", lig_sele="UNK"):
        ...
        self.receptor_indices = top.select("protein")
        self.ligand_indices = top.select("not protein")
        if(len(self.ligand_indices)==0):
            ...
            selection_string = " or ".join([f"resname {aa}" for aa in standard_amino_acids])
            self.receptor_indices = top.select(selection_string)
            self.ligand_indices = top.select("not " + selection_string)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-out", type=str, default="output.csv",
                        help="Output. Default is output.csv \n"
                             ...
                             "per row. ")
    parser.add_argument("-lig", type=str, default="MOL",
                        help="Input, optional. Default is MOL. \n"
