...
def mols2graphs(complex_path, pdbid, label, save_path_l,save_path_p,save_path_aa,save_path_complex, pocket_dis = 5):
    ...
    protein_pdb = os.path.join('./data/v2013', f"{pdbid}", f'{pdbid}_protein.pdb')
...
class GraphDataset(Dataset):
    ...
    def __len__(self):
        return len(self.graph_paths_complex)

if __name__ == '__main__':
    data_root = './data'
    
    data_dir = os.path.join(data_root, 'test2013')
    data_df = pd.read_csv(os.path.join(data_root, 'test2013.csv'))
