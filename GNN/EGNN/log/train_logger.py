...
class TrainLogger(BasicLogger):
    def __init__(self, args,name, config, create=True):
        self.args = args

        timestamp = time.strftime("%Y%m%d_%H%M%S")
        if args.get('mark') == None:
            savetag = timestamp + '_' + args.get('model') + '_' + 'repeat' + str(args.get('repeat'))+ '_' + name
        else:
            savetag = timestamp + '_' + args.get('model') + '_' + 'repeat' + str(args.get('repeat')) + '_' + args.get('mark')+ '_' + name
